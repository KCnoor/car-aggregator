...
// listings = array of { price_sar, source, source_quality_tier?, market_consensus_score? }
// for one (make, model, year, city).
// Returns null if fewer than minSamples (default 5) priced listings.
//
// Weighting combines source tier + market consensus:
//   weight = tier_weight × consensus_multiplier
//   consensus_multiplier:
//     >= 5 sources agree → 1.5
//     >= 3 sources agree → 1.2
//     otherwise          → 1.0
function consensusMultiplier (score) {
...
function computeBaseline (listings, opts = {}) {
  ...
  const weightedSamples = priced.map(l => {
    const tier = tierWeight(l.source_quality_tier ?? 3)
    const consensus = consensusMultiplier(l.market_consensus_score)
    // Round to integer for weightedMedian's sample-expansion.
    const w = Math.max(1, Math.round(tier * consensus))
    return { price: l.price_sar, weight: w }
  })
