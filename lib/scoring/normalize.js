...
// ── AR↔EN translation dictionary ───────────────────────────────────────────
// Loaded once at module init. The legacy haraj-scraper translations.json is
// the canonical source. We index by every Arabic variant (primary + aliases)
// → { slug, en } for O(1) lookup. Categories: makes, models, cities, colors,
// body_types, transmissions, fuel_types, drive_types.
...
function loadTranslations () {
  ...
  try {
    const raw = JSON.parse(fs.readFileSync(TRANSLATIONS_FILE, 'utf8'))
    _translations = {}
    for (const category of Object.keys(raw)) {
      ...
      for (const entry of raw[category]) {
        ...
        const en   = entry.en
        const ar   = entry.ar
        if (ar)   map.set(normalizeArabicForLookup(ar), { slug, en, ar })
        for (const variant of entry.ar_variants ?? []) {
          map.set(normalizeArabicForLookup(variant), { slug, en, ar })
        }
        // Also self-map English aliases so callers can pass either side.
        if (en) map.set(en.toLowerCase(), { slug, en, ar })
      }
      _translations[category] = map
    }
  } catch (e) {
    _translations = {}
  }
...
// Strip Arabic diacritics + collapse whitespace for forgiving lookup.
function normalizeArabicForLookup (s) {
  return String(s)
    .replace(/[ً-ٰٟ]/g, '')   // diacritics
    .replace(/[إأآا]/g, 'ا')                  // alif variants → bare alif
    .replace(/ى/g, 'ي')
    .replace(/ة/g, 'ه')
    .replace(/\s+/g, ' ')
    .trim()
...
function translate (category, input) {
  if (!input) return null
  const t = loadTranslations()
  const map = t[category]
  if (!map) return null
  const key = /[؀-ۿ]/.test(String(input)) ? normalizeArabicForLookup(input) : String(input).toLowerCase()
